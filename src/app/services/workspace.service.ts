...
export class WorkspaceService {
  ...
  public closeWorkspace = (route: string) => {
    const workspaceIndex = this.workspaces.value.findIndex(
      (item) => item.route === route
    );
    ...
    if (this.workspaces.value[workspaceIndex].isActive) {
      ...
      this.activateWorkspace(
        neighborWorkspaceLeft ? neighborWorkspaceLeft.route : '/'
      );
    }

    // Remove workspace
    this.workspaces.next(
      this.workspaces.value.filter((item) => item.route !== route)
    );
  ...
  private activateWorkspace = (route: string) => {
    const workspace = this.workspaces.value.find(
      (item) => item.route === route
    );
  ...
  private loadWorkspaces = () => {
    ...
    const workspaceObject = (JSON.parse(data as string) as Workspaces).map(
      (item) => {
